    ...
    def _validate_shapes(self) -> None:
        ...
        for key in ['pressure', 'saturation']:
            ...
            if shape[2] != self.grid_size or shape[3] != self.grid_size:
                raise ValueError(
                    f"Expected grid size {self.grid_size}×{self.grid_size} for '{key}', "
                    f"got {shape[2]}×{shape[3]}"
                )
...
class PDEBenchADRDataset(BenchmarkDataset):
    ...
    configurable Péclet and Damköhler numbers. Supports 9 parameter
    regimes: Pe ∈ {0.1, 1, 10}, Da ∈ {0.01, 0.1, 1.0}.
    ...
    def _load_data(self) -> None:
        ...
        with h5py.File(self.path, 'r') as f:
            ...
            if 'diffusivity' in f:
                ...
                # Use default diffusivity based on Péclet number
                n_samples = self._end_idx - self._start_idx
            ...
            if 'reaction_rate' in f:
                ...
                # Use default reaction rate based on Damköhler number
                n_samples = self._end_idx - self._start_idx
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dictionary containing:
                ...
                - peclet: Péclet number for this regime
                - damkohler: Damköhler number for this regime
        """
        # Ensure data is loaded
        self._load_data()
